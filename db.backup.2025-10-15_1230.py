...
def search_tasks(conn, user_id, pattern):
    try:
        ...
        cur.execute("""SELECT l.title AS list_title, e.title AS task_title
                       ...
                       WHERE e.user_id=? AND e.type='task' AND e.title LIKE ? AND (e.meta IS NULL OR json_extract(e.meta, '$.deleted') IS NOT TRUE AND json_extract(e.meta, '$.status') != 'done')
...
def convert_entity(conn, user_id, list_name, task_title, new_type):
    try:
        ...
        if not row:
            ...
            return 0
        list_id = row["id"]
        cur.execute("""SELECT id FROM entities WHERE user_id=? AND type='task' AND parent_id=? AND title=? LIMIT 1""", (user_id, list_id, task_title))
        task = cur.fetchone()
        if not task:
            logging.info(f"No task '{task_title}' found in list '{list_name}' for user {user_id}")
            return 0
