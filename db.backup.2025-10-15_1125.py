...
def add_task(conn, user_id, list_name, task_title, entity_type="task"):
    try:
        list_id = _get_or_create_list(conn, user_id, list_name)
        cur = conn.cursor()
        cur.execute("""SELECT id FROM entities WHERE user_id=? AND type=? AND parent_id=? AND title=?""", (user_id, entity_type, list_id, task_title))
        if cur.fetchone():
            logging.info(f"{entity_type.capitalize()} '{task_title}' already exists in list '{list_name}' for user {user_id}")
            return 0
        cur.execute("""INSERT OR IGNORE INTO entities (user_id, type, title, parent_id, meta) VALUES (?, ?, ?, ?, ?)""", 
                    (user_id, entity_type, task_title, list_id, json.dumps({"status": "open"}, ensure_ascii=False)))
        task_id = cur.lastrowid
        logging.info(f"Added {entity_type} '{task_title}' to list '{list_name}' for user {user_id}, ID: {task_id}")
        return task_id
    except sqlite3.Error as e:
        logging.error(f"SQLite error in add_task: {e}")
        raise

def get_list_tasks(conn, user_id, list_name, entity_type="task"):
    try:
        cur = conn.cursor()
        cur.execute("""SELECT id FROM entities WHERE user_id=? AND type='list' AND title=? LIMIT 1""", (user_id, list_name))
        row = cur.fetchone()
        if not row:
            ...
            return []
        list_id = row["id"]
        cur.execute("""SELECT title FROM entities WHERE user_id=? AND type=? AND parent_id=? AND (meta IS NULL OR json_extract(meta, '$.deleted') IS NOT TRUE AND json_extract(meta, '$.status') != 'done') ORDER BY created_at ASC""", 
                    (user_id, entity_type, list_id))
        tasks = [(i+1, r["title"]) for i, r in enumerate(cur.fetchall())]
        logging.info(f"Retrieved {len(tasks)} {entity_type}s for list '{list_name}' for user {user_id}")
        return tasks
    except sqlite3.Error as e:
...
def delete_list(conn, user_id, list_name):
    try:
        ...
        cur.execute("""SELECT id, meta FROM entities WHERE user_id=? AND type IN ('task', 'note') AND parent_id=?""", (user_id, list_id))
...
def get_completed_tasks(conn, user_id):
    try:
        ...
        cur.execute("""SELECT l.title AS list_title, e.title AS task_title, e.meta FROM entities e
                       ...
                       WHERE e.user_id=? AND e.type IN ('task', 'note') AND (json_extract(e.meta, '$.status') = 'done' OR json_extract(e.meta, '$.deleted') = true)
...
def search_tasks(conn, user_id, pattern):
    try:
        ...
        cur.execute("""SELECT l.title AS list_title, e.title AS task_title
                       ...
                       WHERE e.user_id=? AND e.type IN ('task', 'note') AND e.title LIKE ? AND (e.meta IS NULL OR json_extract(e.meta, '$.deleted') IS NOT TRUE AND json_extract(e.meta, '$.status') != 'done')
                       ORDER BY e.created_at ASC""", (user_id, f"%{q}%"))
        tasks = [(row["list_title"], row["task_title"]) for row in cur.fetchall()]
        logging.info(f"Found {len(tasks)} tasks matching '{q}' for user {user_id}")
        return tasks
    except sqlite3.Error as e:
...
def fetch_task(conn, user_id, list_name, task_title):
    try:
        ...
        cur.execute("""SELECT e.id, e.title, e.meta FROM entities e
                       JOIN entities l ON l.id = e.parent_id
                       WHERE e.user_id=? AND e.type IN ('task', 'note') AND l.type='list' AND l.title=? AND e.title=?
...
def fetch_list_by_task(conn, user_id, task_title):
    try:
        ...
        cur.execute("""SELECT l.title AS list_title, e.title AS task_title
                       FROM entities e
                       JOIN entities l ON l.id = e.parent_id
                       WHERE e.user_id=? AND e.type IN ('task', 'note') AND e.title=?
...
def convert_entity(conn, user_id, list_name, task_title, new_type):
    try:
        ...
        if not row:
            ...
            return 0
        list_id = row["id"]
        cur.execute("""SELECT id FROM entities WHERE user_id=? AND type IN ('task', 'note') AND parent_id=? AND title=? LIMIT 1""", (user_id, list_id, task_title))
        task = cur.fetchone()
        if not task:
            logging.info(f"No task '{task_title}' found in list '{list_name}' for user {user_id}")
            return 0
...
def get_all_tasks(conn, user_id):
    try:
        cur = conn.cursor()
        cur.execute("SELECT l.title AS list_title, e.title AS task_title FROM entities e JOIN entities l ON l.id = e.parent_id WHERE e.user_id=? AND e.type IN ('task', 'note') AND (e.meta IS NULL OR json_extract(e.meta, '$.deleted') IS NOT TRUE AND json_extract(e.meta, '$.status') != 'done') ORDER BY l.title, e.created_at", (user_id,))
        tasks = [(row["list_title"], row["task_title"]) for row in cur.fetchall()]
...
def delete_task_fuzzy(conn, user_id, list_name, pattern: str):
    try:
        ...
        if not row:
            ...
            return 0, None
        list_id = row["id"]
        cur.execute("""SELECT id, title, meta FROM entities WHERE user_id=? AND type IN ('task', 'note') AND parent_id=? AND (meta IS NULL OR json_extract(meta, '$.deleted') IS NOT TRUE AND json_extract(meta, '$.status') != 'done')""", 
                    (user_id, list_id))
        candidates = [(r["id"], r["title"], r["meta"]) for r in cur.fetchall()]
        if not candidates:
            logging.info(f"No tasks found in list '{list_name}' for user {user_id}")
...
def delete_task_by_index(conn, user_id, list_name: str, index: int):
    try:
        ...
        if not row:
            ...
            return 0, None
        list_id = row["id"]
        cur.execute("""SELECT id, title, meta FROM entities WHERE user_id=? AND type IN ('task', 'note') AND parent_id=? AND (meta IS NULL OR json_extract(meta, '$.deleted') IS NOT TRUE AND json_extract(meta, '$.status') != 'done') ORDER BY created_at ASC""", 
                    (user_id, list_id))
        tasks = cur.fetchall()
