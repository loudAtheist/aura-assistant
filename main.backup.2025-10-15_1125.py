...
from db import (
    ...
    delete_task_fuzzy, delete_task_by_index, convert_entity, create_list, move_entity, get_all_tasks, update_user_profile, get_user_profile, get_completed_tasks, search_tasks
)
...
SEMANTIC_PROMPT = """
Ты — Aura, дружелюбный и остроумный ассистент, который понимает смысл человеческих фраз и управляет локальной Entity System (списки, задачи, заметки). Ты ведёшь себя как живой помощник: приветствуешь, поддерживаешь, шутишь к месту, переспрашиваешь, если нужно, и всегда действуешь осмысленно.
...
• Команда «Покажи заметки» → показать заметки (action: show_tasks, entity_type: note).
• Если в запросе несколько задач (e.g., «добавь постирать ковер помыть машину»), используй ключ tasks для множественного добавления.
• Решение: создать/добавить/показать/изменить/удалить/отметить/перенести/найти/уточнить/сказать/переименовать/профиль/восстановить/показать выполненные.
...
{{ "action": "create|add_task|show_lists|show_tasks|show_all_tasks|mark_done|delete_task|delete_list|move_entity|convert_entity|search_entity|rename_list|update_profile|restore_task|show_completed_tasks|unknown",
"entity_type": "list|task|note|user_profile",
"list": "имя списка",
"task": "имя задачи",
"to_list": "целевой список для переноса",
"tasks": ["список задач для множественного добавления"],
"meta": {{ "context_used": true, "by_index": 1, "question": "уточняющий вопрос", "reason": "причина действия", "city": "город", "profession": "профессия", "pattern": "поисковый запрос" }} }}
...
• Маркеры завершённости («выполнено», «сделано», «куплено») — mark_done с fuzzy-поиском.
...
• «Переименуй список Покупки в Шопинг» → {{ "action": "rename_list", "entity_type": "list", "list": "Покупки", "task": "Шопинг" }}
• «Преврати молоко в заметку» → {{ "action": "convert_entity", "entity_type": "task", "list": "<последний список>", "task": "Молоко", "meta": {{ "new_type": "note" }} }}
• «Покажи заметки» → {{ "action": "show_tasks", "entity_type": "note" }}
...
async def route_actions(update: Update, context: ContextTypes.DEFAULT_TYPE, actions: list, user_id: int, original_text: str):
    ...
    for obj in actions:
        ...
        entity_type = obj.get("entity_type", "task")  # Default to task
        ...
        if action == "create" and entity_type == "list" and obj.get("list"):
            try:
                ...
                if obj.get("tasks"):
                    for t in obj["tasks"]:
                        add_task(conn, user_id, obj["list"], t, entity_type="task")
            ...
            try:
                ...
                if obj.get("tasks"):
                    for t in obj["tasks"]:
                        task_id = add_task(conn, user_id, list_name, t, entity_type=entity_type or "task")
                        if task_id:
                    ...
                    task_id = add_task(conn, user_id, list_name, task, entity_type=entity_type or "task")
                    if task_id:
        ...
        elif action == "show_tasks" and entity_type:
            try:
                logging.info(f"Showing {entity_type}s for list: {list_name}")
                items = get_list_tasks(conn, user_id, list_name, entity_type=entity_type)
                if items:
                    txt = "\n".join([f"{i}. {t}" for i, t in items])
                    await update.message.reply_text(f"📋 *{list_name}* ({entity_type}s):\n{txt}", parse_mode="Markdown")
                else:
                    await update.message.reply_text(f"Список *{list_name}* ({entity_type}s) пуст.", parse_mode="Markdown")
            ...
            try:
                logging.info("Showing completed/deleted tasks")
                tasks = get_completed_tasks(conn, user_id)
                if not tasks:
                    await update.message.reply_text("Пока нет выполненных или удалённых задач 🕊")
                    set_ctx(user_id, last_action="show_completed_tasks")
                    continue
                txt = "🗂 Выполненные и удалённые задачи:\n"
                for list_title, task_title, meta in tasks:
                    status = "выполнена" if meta.get("status") == "done" else "удалена"
                    txt += f"📋 {list_title}: {task_title} ({status})\n"
                await update.message.reply_text(txt, parse_mode="Markdown")
                set_ctx(user_id, last_action="show_completed_tasks")
            except Exception as e:
            ...
            if name_from_text:
                ...
                items = get_list_tasks(conn, user_id, name_from_text, entity_type="task")
                if items:
                    txt = "\n".join([f"{i}. {t}" for i, t in items])
                    await update.message.reply_text(f"📋 *{name_from_text}* (tasks):\n{txt}", parse_mode="Markdown")
                    set_ctx(user_id, last_action="show_tasks", last_list=name_from_text)
                    continue
                await update.message.reply_text(f"Список *{name_from_text}* (tasks) пуст или не существует.")
